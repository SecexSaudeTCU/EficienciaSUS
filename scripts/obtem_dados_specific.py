...
def obtem_dados_dea(DATABASE_URI, year=2019):
    ...
    # Construção da coluna CNES_SALAS no mês "01" do ano "year"
    df1 = pd.read_sql(f'''SELECT DISTINCT "CNES_ID" AS "CNES",
                          ...
                          WHERE "ANO_ST" = {year} AND "MES_ST" = '01'
                          ORDER BY "CNES_ID"
                       ''', con=engine)

    # Construção da coluna CNES_LEITOS_SUS no mês "01" do ano "year"
    df2 = pd.read_sql(f'''SELECT "CNES_ID" AS "CNES",
                                 CAST(SUM("QT_SUS") AS INTEGER) AS "CNES_LEITOS_SUS"
                          FROM cnes_lt.ltbr
                          WHERE "ANO_LT" = {year} AND "MES_LT" = '01'
                          GROUP BY "CNES_ID"
                          ORDER BY "CNES_ID"
                       ''', con=engine)
    ...
    # Construção da coluna CNES_MEDICOS no mês "01" do ano "year"
    ...
    # Construção da coluna CNES_PROFISSIONAIS_ENFERMAGEM no mês "01" do ano "year"
    ...
    # Construção da coluna FILANTROPICO no mês "01" do ano "year"
    df5 = pd.read_sql(f'''SELECT DISTINCT "CNES_ID" AS "CNES"
                          FROM cnes_ef.efbr
                          WHERE "ANO_EF" = {year} AND "MES_EF" = '01'
                       ''', con=engine)
    ...
    # Construção da coluna SIH_VALOR no ano "year"
    ...
    # Construção da coluna SIA_VALOR no ano "year"
    ...
    # Construção da coluna SIA_SIH_VALOR no ano "year"
    ...
    # Transfere df1 para o arquivo xlsx denominado "dados_para_dea_'year'.xlsx"
    df1.to_excel(os.path.join(consts.DIRETORIO_DADOS, f'dados_para_dea_{year}.xlsx'), index=False)



def obtem_dados_clusterizacao(DATABASE_URI, year=2019):
    ...
    #
    df_hosp_pubs.to_excel(os.path.join(consts.DIRETORIO_DADOS, f'dados_para_clusterizacao_{year}.xlsx'),
                          index=False)


def obtem_dados(DATABASE_URI, year=2019):

    obtem_dados_dea(DATABASE_URI, year)
    obtem_dados_clusterizacao(DATABASE_URI, year)

...
def _retorna_faixa_leitos(num_leitos):
    ...
    elif num_leitos < 101: return '51 a 100'
...
def _trata_dados_para_dea(df, df_cluster):
    ...
    # Insere em "df_hosp_pubs" a coluna SE_GERIDA_OSS antes da coluna TIPO tendo os valores...
    ...
    df_hosp_pubs['SE_GERIDA_OSS'].fillna(value='NA', inplace=True)
...
def prepara_dados_specific(year=2019):

    df_cluster = pd.read_excel(os.path.join(consts.DIRETORIO_DADOS, f'dados_clusterizados_{year}.xlsx'))

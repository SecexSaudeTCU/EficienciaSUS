...
def prepara_dados_specific(ano=2019):

    # Coleta path da pasta de dados em formato zip (que contém um único arquivo "csv")
    arquivo_dados_zip = os.path.join(consts.DIRETORIO_DADOS, f'd{ano}.zip')
    ...
    pos_col_depois_nat_jur = df_tudo.columns.to_list().index('NAT_JURIDICA') + 1
    ...
    # Coleta o index do elemento 'SIA_VALOR' pertencente ao objeto list "df_tudo.columns.to_list()"
    pos_col_valor_sia = df_hosp_pubs.columns.to_list().index('VALOR_SIA')
    # Cria objeto pandas Series que é a soma das colunas SIA_VALOR e SIH_VALOR de "df_tudo"
    valor_total = df_hosp_pubs['VALOR_SIA'] + df_hosp_pubs['VALOR_SIA']
    # Insere em "df_tudo" a coluna SIA_SIH_VALOR antes da coluna SIA_VALOR tendo os valores...
    ...
    df_hosp_pubs = df_hosp_pubs.drop(columns=['GESTAO'])  # Talvez esta coluna seja interessante...
                                                          # para comparar OSS gerida por Estado x Município
    ...
    print(f'Número de estabelecimentos ao final do processamento: {df_hosp_pubs.shape[0]}')

    # Coleta o PATH do arquivo "xlsx" a salvar
    arquivo_para_dea = os.path.join(consts.DIRETORIO_DADOS, f'hosp_pubs_{ano}.xlsx')
